...
def book_slot():
    ...
    rate_per_hour = float(request.form['rate_per_hour'])  # Get the rate from the form
    ...
    amount_paid = int(duration * rate_per_hour)  # Calculate the amount based on rate per hour

    with sqlite3.connect('database.db') as conn:
        cursor = conn.cursor()
        cursor.execute(''' 
            UPDATE parking_slots SET is_occupied = 1, vehicle_number = ?, vehicle_owner = ?, in_time = ?, out_time = ?, payment_status = ?, amount_paid = ? 
            WHERE slot_number = ?''',
